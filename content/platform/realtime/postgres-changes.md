...
Wait for `channel.subscribe()` to finish before writing rows whose notifications you need. A successful subscription means change delivery is ready, including for the first write.

Duplicate notifications are suppressed on a best-effort basis. Re-fetch rows
after reconnecting to catch missed changes.
